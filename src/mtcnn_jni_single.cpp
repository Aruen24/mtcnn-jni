...
Java_mtcnn_Mtcnn_initDetect(JNIEnv *env, jobject instance, jstring detectModelPath_,
    ...
    mMtcnn = new mtcnn(tModelDir, minDetSize, 1);
...
Java_mtcnn_Mtcnn_detect(JNIEnv *env, jobject instance, jbyteArray frame_, jint width_, jint height_) {
...
Java_mtcnn_Mtcnn_setMinSize(JNIEnv *env, jobject instance, jint minSize_) {
    mMtcnn->setMinFace(minSize_);
}
